...
public:
  ...
  SpaceSearcher(const Box3D& bb,
    TIter first, TIter last, T2Point t2p) : SpaceSearcher(bb, first, last,
    thrust::transform_iterator<T2Point, TIter>(first, t2p),
    thrust::transform_iterator<T2Point, TIter>(last, t2p)){}
    // HW4: YOUR CODE HERE

    //struct that defines an ordering on the nodes, based on their codes
    struct  less_than
    {
      bool operator()(morton_pair x, morton_pair y){return x.code_<y.code_;}
    };

// struct that converts points to nodes (with their morton code)
    struct point2morton
    {
      point2morton(MortonCoderType mor) : mor_(mor) {}
      code_type operator()(Point poi) {return mor_.code(poi);}
      MortonCoderType mor_;
    };
  ...
  template <typename TIter, typename PointIter>
    ...
    // HW4: YOUR CODE HERE
// convert points into morton code
      thrust::transform_iterator<point2morton,PointIter> first_code(pfirst,point2morton(mc_));
      thrust::transform_iterator<point2morton,PointIter> last_code(plast,point2morton(mc_));
//convert into tuple by making zip iterator of tuple iterators

      typedef thrust::zip_iterator<thrust::tuple<thrust::transform_iterator<point2morton,PointIter>,TIter>> zip_it;
      zip_it zfirst(thrust::make_tuple(first_code,tfirst));
      zip_it zlast(thrust::make_tuple(last_code,tlast));

      //initiate z data
      z_data_ = std::vector<morton_pair> (zfirst,zlast); 
      thrust::sort(thrust::omp::par,z_data_.begin(),z_data_.end(),less_than());
    }
  ...
  // A (code_type,value_type) pair that can be used as a MortonCode
    ...
    // HW4: YOUR CODE HERE
      // get the data from the morton tuple
      morton_pair(thrust::tuple<code_type, T> morton_tuple)
      {
        code_ = thrust::get<0>(morton_tuple);
        value_ = thrust::get<1>(morton_tuple);
      }
    };
